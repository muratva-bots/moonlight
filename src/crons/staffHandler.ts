...
function staffControl({ client, guild }: { client: Client; guild: Guild }) {
    schedule('0 0 0 * * 7', async () => {
        ...
        if (!guildData || !guildData.point.ranks?.length) return;

        const minStaffRole = guild.roles.cache.find((r) => r.name === guildData.moderation.minStaffRole);
        ...
        const documents = await StaffModel.find({
            id: { $in: members },
            $or: [
                { roleTime: { $gte: now + ONE_DAY * 3 } },
                { staffTime: { $gte: now + ONE_DAY * 7 } },
            ],
        });
        if (!documents.length) return;

        const sortedRanks = guildData.point.ranks.slice().sort((a, b) => a.point - b.point);
        for (const document of documents) {
            if (document.lastWeekPoints !== 0 && 3000 > document.allPoints - document.lastWeekPoints) {
                ...
                if (currentIndex === 1) {
                    ...
                    logChannel.send({
                        embeds: [
                            embed.setDescription(`${member} (${inlineCode(member.id)}) adlı kullanıcı ${currentRole} (${inlineCode(currentRole.id)}) rolündeydi daha düşürelecek yetkisi olmadığından yetkiden atıldı.`)
                        ]
                    });
                ...
                if (sortedRanks[currentIndex].extraRole !== sortedRanks[currentIndex - 1].extraRole) {
                    if (!member.roles.cache.has(sortedRanks[currentIndex].extraRole)) await member.roles.remove(sortedRanks[currentIndex].extraRole);
                    if (!member.roles.cache.has(sortedRanks[currentIndex - 1].extraRole)) await member.roles.add(sortedRanks[currentIndex - 1].extraRole);
                }

                logChannel.send({
                    embeds: [
                        embed.setDescription(`${member} (${inlineCode(member.id)}) adlı kullanıcı ${currentRole} (${inlineCode(
                            currentRole.id,
