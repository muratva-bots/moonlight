...
function staffControl({ client, guild }: { client: Client, guild: Guild }) {
    schedule("0 0 0 * * 7", async () => {
        ...
        const members = guild.members.cache.filter(m => m.roles.highest.position >= minStaffRole.position).map(m => m.id);
        const now = Date.now();
        ...
        for (const document of documents) {
            if (document.lastWeekPoints !== 0 && 3000 > (document.allPoints - document.lastWeekPoints)) {
                ...
                const currentIndex = ranks.findIndex((t) => member.roles.cache.map((r) => r.name).some((r) => t.ROLE === r));
                if (!currentIndex) continue;

                if (ranks[currentIndex].ROLE === "Thoosa of the Poseidon") {
                    member.roles.remove(member.roles.cache.filter(r => r.position >= minStaffRole.position).map(r => r.id));
                    logChannel.send(`${member} (${inlineCode(member.id)}) adlı kullanıcı ${bold("Thoosa of the Poseidon")} rolündeydi daha düşürelecek yetkisi olmadığından yetkiden atıldı.`);
                ...
                logChannel.send(`${member} (${inlineCode(member.id)}) adlı kullanıcı ${bold(currentRole.name)} (${inlineCode(currentRole.id)}) rolünden ${bold(newRole.name)} (${inlineCode(newRole.id)}) rolüne düşürüldü.`);
