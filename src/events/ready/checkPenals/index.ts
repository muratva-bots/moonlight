...
export async function checkPenals(client: Client, guild: Guild) {
    ...
    const now = Date.now();
    const specialCommands = (guildData.moderation.specialCommands || []).filter(p => p.type === SpecialCommandFlags.Punishment);
...
export function checkBannedTag(client: Client, member: GuildMember, guildData: ModerationClass) {
    if (!guildData.bannedTags || !guildData.bannedTags.length || !member.guild.roles.cache.has(guildData.bannedTagRole)) return false;
